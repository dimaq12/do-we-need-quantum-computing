...
\title{\textbf{Do We Really Need Quantum Computing?}}
...
wearing a quantum costume? It turns out there is a single number you can measure,
matrix-free, that tells you which side of the line you are on.

\section*{The idea}

The number is \textbf{effective rank}---in \texttt{resona}, \Phione. The shorthand:
\emph{quantum advantage needs structure a classical machine cannot cheaply
simulate.} And the most common ``structure'' in quantum ML---low rank---is exactly
the structure classical sampling harvests for free.
...
\]
\textbf{Low \Phione{} $\Rightarrow$ dequantizable.} \textbf{High \Phione{}
$\Rightarrow$ genuine frontier}, no sampling handle, no shortcut. One dial,
computed from matrix--vector products, with no matrix ever formed.
...
\emph{structure} does the problem have?'', and structure is measurable. A quantum
computer is a machine that pays a fixed price (qubits, coherence, error
correction) to handle high-structure problems. If the structure is low, you
overpaid: a classical sampler does the same job for free.

\section*{The demo: the dequantizable side}
...
paying for low-rank structure that classical sampling extracts for free.

\section*{The demo: the genuine wall}
...
than a simple periodic signal. And the extractability test---does the structure
...
classically. The Shor target keeps growing ($1.5 \to 40.3$) and never closes:
\texttt{extractable? False}. The same dial that dequantized the low-rank problem
here points firmly the other way---\emph{from the same matrix-free measurement}.
